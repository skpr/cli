...
func (c *Chart) Render() string {
	...
	if c.title != "" {
		...
		buf.WriteString("\033[22m") // reset bold only (preserve background)
		buf.WriteString("\n")
		if c.titleColor != "" {
		...
		if c.titleColor != "" {
			buf.WriteString("\033[39m") // reset foreground only (preserve background)
		}
		buf.WriteString("\n")
	}
	...
	reset := "\033[39m" // reset foreground only (preserve background)
